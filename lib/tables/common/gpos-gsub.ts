...
export function parseCoverageTable(reader: DataReader, offset: number): number[] {
    ...
    try {
        ...
        if (coverageFormat === 1) {
            ...
            throw new Error(`対応していないカバレッジフォーマット: ${coverageFormat}`);
        }
    } finally {
        // 位置を復元
        reader.restore();
...
export function parseClassDefTable(reader: DataReader, offset: number): ClassDefTable {
    ...
    try {
        ...
        if (classFormat === 1) {
            ...
            throw new Error(`対応していないクラス定義フォーマット: ${classFormat}`);
        }
    } finally {
        // 位置を復元
        reader.restore();
 ...
 */
export function parseLookupListTable(reader: DataReader, offset: number, parseLookupTable: Function) {
    // 位置を保存
    reader.save();
    ...
    try {
        ...
        for (const lookupOffset of lookupOffsets) {
            lookupTables.push(parseLookupTable(reader, lookupOffset));
        }
