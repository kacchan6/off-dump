...
import { hasCffOrCff2Table, getCffOrCff2Table, extractGlyphPaths } from './cff-utils-common';
import { PathCommand } from '../../tables/cff-interpreter';
...
export function analyzeGlyphSymmetry(path: PathCommand[]): {
  ...
  if (points.length === 0) {
    return { horizontalSymmetry: 0, verticalSymmetry: 0 };
  }
  
  // 水平方向の対称性をチェック
  let horizontalSymmetryScore = 0;
  let verticalSymmetryScore/**
 * CFF/CFF2テーブルのグリフメトリクスと統計分析のユーティリティ関数
 */
...
export function getGlyphMetricsDetail(font: Font, glyphId: number): object | null {
  ...
  return {
    ...
    context: {
      ...
      closes: { min: Number.POSITIVE_INFINITY, max: Number.NEGATIVE_INFINITY, avg: